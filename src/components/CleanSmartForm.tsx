...
const CleanSmartForm: React.FC = () => {
  ...
  const { isLoading, isSuccess, isError, errorMessage, trigger, reset: resetWebhook } = useN8nWebhook()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    /* Read the CSV as plain text to include in the JSON payload sent to n8n */
    const csvContent = await csvFile.text()
    await trigger({ type: agencyType, agent, csvFileName: csvFile.name, csvContent })
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-3">
      ...
      <div className="space-y-1.5">
        {/* Zone d'upload — visuellement séparée avec bordure en tirets */}
        <button
          type="button"
        ...
        >
          {csvFile ? (
            /* Loaded state — vert avec icône check et nom de fichier */
        ...
        {/* Bouton "changer" discret quand un fichier est déjà chargé */}
        {csvFile && (
          <button
            type="button"
...
function TabGroup<T extends string>({ options, selected, onSelect }: TabGroupProps<T>) {
  return (
    /*
     * Container avec bordure grise arrondie et padding interne.
     * Les deux tabs s'inscrivent dedans et remplissent chaque moitié.
     */
